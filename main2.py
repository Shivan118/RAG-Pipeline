...
# Add professional styling with better mobile support
st.markdown("""
    <style>
    /* Base text colors */
    .stApp {
    ...
    /* Mobile-responsive chat container styling */
    .chat-container {
        display: flex;
    ...
    .chat-history {
        ...
        overflow-y: auto;
        padding: 10px;
        margin-bottom: 60px;  /* Reduced space for input box */
    }

    /* Mobile-responsive fixed chat input */
    .chat-input-fixed {
        position: fixed;
        bottom: 0;
        left: 0;
    ...
    /* Ensure chat messages are visible and responsive */
    .stChatMessage {
        background-color: #ffffff !important;
        border: 1px solid #E9ECEF !important;
    ...
    /* Mobile-friendly chat interface styling */
    .chat-interface {
        display: flex;
        flex-direction: column;
    ...
    /* Mobile-friendly avatar styling */
    .avatar {
        width: 24px;
        height: 24px;
        border-radius: 4px;
    ...
    /* Role label styling - smaller for mobile */
    .role-label {
        font-weight: 600;
        margin-bottom: 4px;
        color: #1E3D59;
    ...
    /* Source styling - more compact for mobile */
    .source-container {
        background-color: #f7f7f8;
        border: 1px solid #e5e5e5;
        border-radius: 8px;
    ...
    @media (max-width: 768px) {
        ...
        /* Ensure main content is usable on mobile */
...
# Add a toggle button for mobile users
col1, col2 = st.columns([6, 1])
with col2:
    if st.button("📂", help="Toggle document panel"):
        toggle_sidebar()

# Sidebar for document upload and management
sidebar_visibility = "visible" if st.session_state.sidebar_state == 'expanded' else "collapse"

with st.sidebar:
    st.markdown(f"""
        <div style="visibility: {sidebar_visibility};">
            <h3 style="font-size: 1.2rem; margin-bottom: 0.5rem;">📂 Document Management</h3>
        </div>
    """, unsafe_allow_html=True)
        ...
        if sources:
            with st.expander("View Sources", expanded=False):
                for i, source in enumerate(sources, 1):
                    ...
                    if len(content) > 300:  # Reduced for mobile
                        content = content[:300] + "..."
                    st.code(content)

st.markdown("</div>", unsafe_allow_html=True)

# Chat input (placed at bottom with mobile-friendly styling)
st.markdown("""
    ...
    with st.spinner("Thinking..."):
        try:
            ...
            for i in range(len(response_text)):
                ...
                """, unsafe_allow_html=True)
                if i % 3 == 0:  # Adjusted for mobile (faster typing)
                    time.sleep(0.005)  # Faster typing speed for mobile
